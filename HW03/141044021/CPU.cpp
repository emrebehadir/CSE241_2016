...
void CPU::print() const {
    if (opinion=='1') {
        cout <<statement.opCode <<" " <<setw(3)
             ...
             <<statement.operandTwo<<"- ";     
        for (int i = 0; i < 5; i++)
            cout <<setw(1)<<"R" <<i+1<< "=" <<setw(3)<<  registers[i]<< " ";
        cout<<"PC= "<<PC << endl;                
    }
...
void CPU::execute(const string instruction) {
    setInst();
